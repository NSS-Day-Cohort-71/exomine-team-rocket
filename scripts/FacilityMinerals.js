...
export async function fetchFacilityMinerals(facilityId) {
    let response = await fetch(`http://localhost:8088/facilityMinerals?facilityId=${facilityId}`);
...
export function handleFacilityDropdownChange(facilities) {
    ...
    facilitiesDropdown.addEventListener('change', async function(event) {
        ...
        if (facilityId !== 0) {
            ...
            let facilityMineralsHTML = renderFacilityMineralsHTML(facilityName, facilityMinerals);
